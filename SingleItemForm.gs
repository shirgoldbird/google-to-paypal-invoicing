...
function parseAllItems() {
  ...
  var itemNameRow = findTextInColumn(configSheet, 1, 'Item Name', false).getRow() - 1;
  var itemName = config[itemNameRow][1];
  documentProperties.setProperty(configObjLookup['Item Name'], itemName);

  var itemPriceRow = findTextInColumn(configSheet, 1, 'Item Price', false).getRow() - 1;
  var itemPrice = config[itemPriceRow][1];
  documentProperties.setProperty(configObjLookup['Item Price'], itemPrice);

  Logger.log(documentProperties.getProperty(configObjLookup['Item Price']));
  Logger.log(documentProperties.getProperty(configObjLookup['Item Name']));

  // we already have this item
  if (!findTextInColumn(inventorySheet, 1, itemName, false)) {
    inventorySheet.appendRow([itemName, itemPrice, 0]);
  }
}
...
// hardcode 1 order of whatever we have for sale
function parseOrder(rowId) {
  var rowOrder = [];
  var order = {};
  order['name'] = documentProperties.getProperty(configObjLookup['Item Name']);
  order['quantity'] = 1;
  order['unit_amount'] = {
    'currency_code': 'USD',
    'value': documentProperties.getProperty(configObjLookup['Item Price'])
  };
