...
// parses a row representing an item named like "Applejack - $5 each"
// `updateProperties` should be set to true when the inventory is being generated or rebuilt
// updating the property store is rate limited by Google, so it's important to not needlessly update it
function parseItemInfo(item, updateProperties=false) {
  var itemInfo = {};
  var itemType = "Stickers";
  var splitText = item.split(" - ");
  var price;
  var itemName;
  var itemNameForInvoice;

  if (item == "Complete Mane Six Set for $25! One of Each!") {
    price = {
      1: "25",
      2: "22.50",
      3: "21.66",
    ...
    //price = "25";
    itemName = "Complete Mane Six Set";
    itemNameForInvoice = "Complete Mane Six Sticker Set";
  }
  // if there's a hypen it's an item, otherwise it's not
    ...
    itemName = splitText[0]; // Applejack
    itemNameForInvoice = itemName + " Sticker"; 
  } else {
    return null;
  }
...
function parseAllItems() {
  ...
  for (var i = 0; i < orderHeaders.length; i++) {
    ...
    // set updateProperties to true so we update our store
    var itemObj = parseItemInfo(item, true);
    Logger.log(item)
    Logger.log(itemObj)
    // check if parseItemInfo found an item
    if (itemObj && itemObj[orderHeaders[i]]) {
      // we already have this item
      if (findTextInColumn(inventorySheet, 1, itemObj[item]['itemName'], false)) {
        continue;
      } else {
        inventorySheet.appendRow([itemObj[item].itemName, itemObj[item].price['1'], 0]);
      }
    }
  }
}
...
function parseOrder(rowId) {
  ...
  for (var i = 0; i < orders[orderIndex].length; i++) {
    ...
    var item = orderHeaders[i];
    // don't update the properties store, so we're not needlessly rate limited
    ...
    if (currCell !== "None" && currCell !== "" && !isEmpty(itemObj)) {
      ...
      if (Number.isInteger(currCell)) {
        quantity = currCell;
      } else if (itemObj[item].itemName == "Complete Mane Six Sticker Set") {
        var quantityStr = currCell.split(" Set")[0];
        quantity = parseInt(quantityStr);
      }
      else {
        var quantityStr = currCell.split(" Sticker")[0];
        quantity = parseInt(quantityStr);
      }
      price = itemObj[item].price[quantity];
...
function getToken() {
  ...
  */
  ...
  Logger.log('attempted to generate token, response was %s', parsedResponse);
...
function callPaypalUrl(path, payload) {
  ...
  Logger.log("PayPal URL %s called, reponse is: %s", path, parsedResponse);
...
function onFormSubmit(e) {
  ...
  var invoiceUrl = sendInvoice(order);
  if (invoiceUrl && invoiceUrl != "") {
    var invoiceUrlColumnId = getColumnIndexFromName(orderHeaders, 'PayPal Invoice');
    orderSheet.getRange(e.range.getRow(), invoiceUrlColumnId + 1).setValue(invoiceUrl);
  }
}
...
function test_onFormSubmit_Single() {
  var rowId = 2; // enter the row ID as displayed on the spreadsheet. (For example, Row 1 will always be the spreadsheet header) 
  if (SpreadsheetApp.getActiveSheet().getName() !== "Form Responses 1") { Logger.log("Please change the active spreadsheet tab to 'Form Responses 1' and rerun."); return; }
  var order = {};
  order['items'] = parseOrder(rowId);
  order['buyer'] = parseBuyerInfo(rowId);
  var invoiceUrl = sendInvoice(order);
  if (invoiceUrl && invoiceUrl != "") {
    var invoiceUrlColumnId = getColumnIndexFromName(orderHeaders, 'PayPal Invoice');
    orderSheet.getRange(rowId, invoiceUrlColumnId + 1).setValue(invoiceUrl);
  }
}
